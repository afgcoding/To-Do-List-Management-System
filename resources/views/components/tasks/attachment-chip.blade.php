...
@if ($attachment->isImage())
    <div x-data="{ lightbox: false }" class="mt-3 w-full max-w-md overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
...
@else
    <div class="mt-3 flex w-full max-w-md items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 p-2.5 transition hover:bg-slate-100/80">
        <div class="flex min-w-0 items-center gap-2.5">
            <span class="grid size-9 shrink-0 place-items-center rounded-md text-[10px] font-bold uppercase {{ $iconWrap }}">{{ $type }}</span>
            <div class="min-w-0">
                <p class="truncate text-xs font-semibold text-slate-800">{{ $attachment->file_name }}</p>
