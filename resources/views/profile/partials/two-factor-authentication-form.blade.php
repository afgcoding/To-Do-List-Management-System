<section>
    ...
    @if (session('two_factor.recovery_codes'))
        <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <ul class="mt-3 grid grid-cols-2 gap-2 font-mono text-sm text-slate-800">
    ...
    @elseif (filled($user->two_factor_secret))
        <div class="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            @if ($user->twoFactorQrImageUrl())
                <img src="{{ $user->twoFactorQrImageUrl() }}" alt="Two-factor QR code" class="mt-4 h-48 w-48 rounded-lg border border-slate-200 bg-white p-2">
            @endif
            <p class="mt-3 text-xs font-medium uppercase tracking-wide text-slate-500">Manual key</p>
            <p class="mt-1 font-mono text-sm tracking-widest text-slate-800">{{ $user->twoFactorSecretPlain() }}</p>
