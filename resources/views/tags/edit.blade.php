...
@section('content')
<div class="mx-auto max-w-xl" x-data="{ name: @js(old('name', $tag->name)), color: @js(old('color', $tag->color ?: '#6366F1')) }">
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 bg-slate-50 px-6 py-4">
            <h1 class="text-lg font-semibold text-slate-900">Edit tag</h1>
            <p class="mt-1 text-sm text-slate-500">Give the tag a unique name and a color.</p>
        </div>
        ...
        <form method="POST" action="{{ route('tags.update', $tag) }}" class="space-y-5 p-6">
            ...
            <div class="space-y-1.5">
                ...
                <input id="name" name="name" x-model="name" required placeholder="Bug, Feature, Urgent-Fix" class="{{ $field }}">
