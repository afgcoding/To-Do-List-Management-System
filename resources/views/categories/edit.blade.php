...
@section('content')
<div class="mx-auto max-w-xl space-y-4" x-data="{ name: @js(old('name', $category->name)), color: @js(old('color', $category->color ?: '#6366F1')) }">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form method="POST" action="{{ route('categories.update', $category) }}" class="space-y-5 p-6">
            @csrf
            @method('PUT')
